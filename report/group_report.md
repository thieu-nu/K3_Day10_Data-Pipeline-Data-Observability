...
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `C1-BLOCKER-001` | `raw_count = N/A`, `clean_count = N/A`; chưa thể tính chênh lệch. `max_results=24` chỉ là giới hạn request, không phải raw count quan sát được. | Machine-readable evidence: `data/quality/clean_contract_gate.json` ghi **STOP** và năm mã thiếu artifact. `data/raw/` và `data/clean/` hiện chỉ có `.gitkeep`; ingestion/cleaning vẫn dừng bằng `NotImplementedError`. | Gate **STOP/CLOSED**. Tuấn tạo raw response/records; Mạnh tạo clean CSV/JSON và audit theo `clean-v1`; Quân chạy `python script/check_clean_contract.py`. Chỉ khi report chuyển **GO**, `phase1.py` mới gọi index/test set. |
